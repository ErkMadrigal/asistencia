...
class UbicacionModel
{
    ...
    public function GetUbicacionById($id){
        $builder = $this->db->table('ubicacion');
        $builder->select("razon_social AS idCliente,cliente.nombre_corto,nombre_ubicacion,ubicacion.activo,idCodigoPostal,calle_num,colonia,municipio,ciudad,idEstado, ubicacion.createddate, ubicacion.updateddate,CONCAT(UA.nombre,' ' ,UA.apellido_paterno) AS createdby,CONCAT(UU.nombre,' ' ,UU.apellido_paterno) AS updatedby");
       $builder->join("cliente","ubicacion.idCliente = cliente.id","left"); 
